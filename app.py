...
def generate_invoice(csv_path):
    ...
    for col_idx in range(1, total_columns + 1):
        ...
        max_length = max(len(str(cell.value)) for row in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=col_idx, max_col=col_idx) for cell in row if cell.value)
    ...
    # Preview data
    preview_data = df_sorted.head(5).to_html(classes='preview-table', index=False)

    return excel_filename, pdf_filename, preview_data

# ---------------- Routes ----------------
@app.route("/", methods=["GET", "POST"])
def index():
    excel_file = pdf_file = preview_data = None
    error = None
    ...
    return render_template("index.html", excel_file=excel_file, pdf_file=pdf_file,
                           preview_data=preview_data, error=error)
